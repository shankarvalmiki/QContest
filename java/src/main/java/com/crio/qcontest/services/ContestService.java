...
public class ContestService{
    ...
    public Contestant createContestant(Long contestId, String userName) {
        ...
        if((contest.get().getContestStatus()==ContestStatus.ENDED)){
            throw new RuntimeException();
        }
    ...
    public class comparator implements Comparator<Contestant>{
        @Override 
        public int compare(Contestant user1,Contestant user2){
            return user2.getCurrentContestPoints()-user1.getCurrentContestPoints();
        }
    ...
    public List<Contestant> contestHistory(Long contestId) {
        ...
        List<Contestant> res=contestantRepository.findAllByContestId(contestId);
